...
# https://github.com/hank-nguyen/MBT-optimizer/blob/master/lr_backtrack.py
################
...
class LRFinder(object):
    ...
    def backtrack(self, trainloader, alpha=0.5, beta=0.5, num_iter=20, lr_justified=True):
        ...
        # final ouput of learning rate finder
        self.lr_BT = np.mean(self.history['lr'])
    ...
    def _train_batch(self, inputs, labels, lr_current):
        ...
        loss.backward()
        self.loss = loss.item()
        self.norm_squared = np.sum(
            [torch.norm(param.grad, p=2).item()**2 for param in self.params])
    ...
    def track_batch(self, inputs, labels):
        ...
        self.check = self.loss_new - self.loss + \
            self.alpha*self.lr_current*self.norm_squared
